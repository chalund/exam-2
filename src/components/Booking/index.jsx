...
const VenueBookings = () => {
  const { id } = useParams();
  const { data, loading, error } = useFetch(`${BASE_URL}${Venues}/${id}?_bookings=true`);
  ...
  return (
    <div className="mt-6">
      <h1 className="text-center text-xl text-violet-600 uppercase">{venueName} Bookings</h1>
      <ul>
        {data.data.bookings.length === 0 ? (
          <li className="text-center">No bookings yet in this venue.</li>
        ) : (
          data.data.bookings.map((booking) => {
            const { numberOfDays, totalCost } = calculateCostDetails(booking.dateFrom, booking.dateTo, pricePerNight);

            return (
              <li key={booking.id} className="border border-black m-2 p-2">
